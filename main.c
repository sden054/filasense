...
// 32 below didnt work as 7 seconds (chinese clone?). +20 (poke method) did it.
...
interrupt [TIM0_OVF] void timer0_ovf_isr(void)
{
	...
	if(BTN==0){
		...
		if(trig){
      		// button released
      		if(running || alarmset){ // if running or alarm set
      			running=0;
      			PWR=0;
      		}else{ // start from not running only (no alarm)
      			running=1;
      			PWR=1;
      		}
			ALM=1; // reset alarm
